...
export default class Method {
    constructor(contract, abi) {
        this.unichainJS = contract.unichainJS;
        ...
        this.signature = this.unichainJS.sha3(this.functionSelector, false).slice(0, 8);
    ...
    onMethod(...args) {
        ...
        args.forEach((arg, index) => {
            if (types[index] == 'address')
                args[index] = this.unichainJS.address.toHex(arg).replace(ADDRESS_PREFIX_REGEX, '0x')

            if (types[index] == 'address[]') {
                args[index] = args[index].map(address => {
                    return this.unichainJS.address.toHex(address).replace(ADDRESS_PREFIX_REGEX, '0x')
                })
            }
        });
    ...
    async _call(types, args, options = {}, callback = false) {
        ...
        this.unichainJS.transactionBuilder.triggerSmartContract(
            this.contract.address,
            this.functionSelector,
            options,
            parameters,
            options.from ? this.unichainJS.address.toHex(options.from) : false,
            (err, transaction) => {
                if (err)
                    return callback(err);
                    ...
                    if (len === 0 || len % 64 === 8) {
                        ...
                        if (len !== 0) {
                            ...
                            for (let i = 0; i < len - 8; i += 64) {
                                msg2 += this.unichainJS.toUtf8(chunk.substring(i, i + 64))
                            }
    ...
    async _send(types, args, options = {}, privateKey = this.unichainJS.defaultPrivateKey, callback = false) {
        ...
        try {
            const address = privateKey ? this.unichainJS.address.fromPrivateKey(privateKey) : this.unichainJS.defaultAddress.base58;
            const transaction = await this.unichainJS.transactionBuilder.triggerSmartContract(
                this.contract.address,
                this.functionSelector,
                options,
                parameters,
                this.unichainJS.address.toHex(address)
            );
            ...
            if (broadcast.code) {
                ...
                if (broadcast.message)
                    err.message = this.unichainJS.toUtf8(broadcast.message);
                return callback(err)
            }
            ...
            const checkResult = async (index = 0) => {
                ...
                if (output.result && output.result === 'FAILED') {
                    return callback({
                        error: this.unichainJS.toUtf8(output.resMessage),
                        transaction: signedTransaction,
                        output
                    });
                }
    ...
    async _watch(options = {}, callback = false) {
        ...
        if (!this.unichainJS.eventServer)
        ...
        const getEvents = async () => {
            try {
                ...
                const events = await this.unichainJS.event.getEventsByContractAddress(this.contract.address, params);
