...
export default class Event {
    ...
    setServer(eventServer = false, healthcheck = 'healthcheck') {
        if (!eventServer)
            return this.unichainJS.eventServer = false;

        if (utils.isString(eventServer))
            eventServer = new providers.HttpProvider(eventServer);

        if (!this.unichainJS.isValidProvider(eventServer))
            throw new Error('Invalid event server provided');

        this.unichainJS.eventServer = eventServer;
        this.unichainJS.eventServer.isConnected = () => this.unichainJS.eventServer.request(healthcheck).then(() => true).catch(() => false);
    }
    ...
    getEventsByContractAddress(contractAddress = false, options = {}, callback = false) {
        ...
        if (contractAddress)
            routeParams.push(this.unichainJS.address.fromHex(contractAddress));
        ...
        return this.unichainJS.eventServer.request(`event/contract/${routeParams.join('/')}?${querystring.stringify(qs)}`).then((data = false) => {
            if (!data)
    ...
    getEventsByTransactionID(transactionID = false, options = {}, callback = false) {
        ...
        return this.unichainJS.eventServer.request(`event/transaction/${transactionID}`).then((data = false) => {
            if (!data)
                return callback('Unknown error occurred');
