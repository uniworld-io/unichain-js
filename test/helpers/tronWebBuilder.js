...
const UnichainJS = require('../setup/UnichainJS');
...
const newTestAccounts = async (amount) => {
    const unichainJS = createInstance();

    console.log(chalk.blue(`Generating ${amount} new accounts...`))
    await unichainJS.fullNode.request('/admin/temporary-accounts-generation?accounts=' + amount);
...
const getTestAccounts = async (block) => {
    ...
    const unichainJS = createInstance();
    const accountsJson = await unichainJS.fullNode.request('/admin/accounts-json');
    ...
    for (let i = 0; i < accounts.pks.length; i++) {
        let addr = unichainJS.address.fromPrivateKey(accounts.pks[i]);
        accounts.b58.push(addr);
        accounts.hex.push(unichainJS.address.toHex(addr));
    }
